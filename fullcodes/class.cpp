    ...
    */
    for (int i = 0; i < sum_building; i++) {
        if (i <= 44) {
            ...
            all_building[i]->setInitialPolicy();
        }
        if (i >= 45 && i <= 50) {
    ...
    for (int i = 0; i < 40000; i++) {
        ...
        for (int j = 8; j <= 9; j++) {
            ...
            else if (0.7 <= P && P < 0.9) { all_person[i]->track[j] = 58; }
            else if (0.9 <= P && P <= 1.0) { all_person[i]->track[j] = 63; }
        }
        ...
        for (int j = 10; j <= 11; j++) {
            ...
            else if (0.6 <= P && P < 0.75) { all_person[i]->track[j] = 58; }
            else if (0.75 <= P && P < 0.85) { all_person[i]->track[j] = temp_lunchbuilding1; }
            else if (0.85 <= P && P < 0.95) { all_person[i]->track[j] = 63; }
        ...
        else if (0.75 <= P && P < 0.85) { all_person[i]->track[12] = 58; }
        else if (0.85 <= P && P < 0.97) { all_person[i]->track[12] = 63; }
        ...
        else if (0.6 <= P && P < 0.75) { all_person[i]->track[17] = 58; }
        else if (0.75 <= P && P < 0.9) { all_person[i]->track[17] = temp_dormitory; }
        else if (0.9 <= P && P < 0.95) { all_person[i]->track[17] = 60; }
        else if (0.95 <= P && P < 0.98) { all_person[i]->track[17] = 63; }
        ...
        else if (0.5 <= P && P < 0.7) { all_person[i]->track[18] = 58; }
        else if (0.7 <= P && P < 0.85) { all_person[i]->track[18] = temp_dormitory; }
        else if (0.85 <= P && P < 0.9) { all_person[i]->track[18] = 60; }
        else if (0.9 <= P && P < 0.95) { all_person[i]->track[18] = 63; }
        ...
        for (int j = 19; j <= 20; j++) {
            ...
            else if (0.4 <= P && P < 0.6) { all_person[i]->track[j] = 58; }
            else if (0.6 <= P && P < 0.8) { all_person[i]->track[j] = temp_dormitory; }
            else if (0.8 <= P && P < 0.86) { all_person[i]->track[j] = 60; }
            else if (0.86 <= P && P < 0.93) { all_person[i]->track[j] = 63; }
        ...
        else if (0.6 <= P && P < 0.68) { all_person[i]->track[21] = 60; }
        else if (0.68 <= P && P < 0.76) { all_person[i]->track[21] = 63; }
        else if (0.76 <= P && P < 0.85) { all_person[i]->track[21] = temp_gymnasium8; }
        else if (0.85 <= P && P <= 1.0) { all_person[i]->track[21] = 58; }
...
void Game::startSpread() {
    ...
    for (int h = 0; h < 24; h++) {
        for (int i = 0; i < sum_healthy_person + sum_ill_person + sum_quarantine_person; i++) {
            ...
            all_building[all_person[i]->track[h]]->peopleArrived(1, all_person[i]);
            if (all_person[i]->health_state == 1)
                all_building[all_person[i]->track[h]]->infectedPeopleArrived(1);
        }
        for (int i = 0; i < sum_healthy_person + sum_ill_person + sum_quarantine_person; i++) {
            ...
            if (rand_p <= p) {
                all_person[i]->is_now_infected = true;
                sum_ill_person++;
                sum_healthy_person--;
                new_infected++;
            }
...
void Game::updateTrack() {
    for (int i = 0; i < sum_healthy_person + sum_ill_person + sum_quarantine_person; i++) {
        int before_idx = all_person[i]->track[0];
        for (int h = 0; h < 24; h++) {
            if (all_building[all_person[i]->track[h]]->isDormitoryShutDown()) {
                ...
                all_person[i]->track[h] = before_idx;
            }
            else {
...
void Building::setInitialPolicy() {
    ...
    addPolicy(new RequireMasks(0, 10, this));
    addPolicy(new MeasureTemperatureAndScanCode(1, 20, this));
    addPolicy(new ShutDownBuilding(2, 30, this));
    updatePolicyState();
}
...
void DiningHall::setInitialPolicy() {
    addPolicy(new AddBaffle(3, 20, this));
    addPolicy(new RequireMasks(0, 20, this));
    addPolicy(new MeasureTemperatureAndScanCode(1, 30, this));
    addPolicy(new StopDineIn(4, 40, this));
    updatePolicyState();
}
...
void TeachingBuilding::setInitialPolicy() {
    addPolicy(new RequireMasks(0, 20, this));
    addPolicy(new MeasureTemperatureAndScanCode(1, 30, this));
    addPolicy(new ShutDownBuilding(2, 40, this));
    updatePolicyState();
}
...
void Dormitory::setInitialPolicy() {
    addPolicy(new RequireMasks(0, 20, this));
    addPolicy(new TemporaryLockdown(5, 30, this));
...
void RequireMasks::takeEffect() {
    BuildingAttached()->updateChange(-0.00003);
    BuildingAttached()->updateActionPoint(1);
}
...
void MeasureTemperatureAndScanCode::takeEffect() {
    BuildingAttached()->updateChange(-0.00002);
    BuildingAttached()->updateActionPoint(2);
}
...
void ShutDownBuilding::takeEffect() {
    BuildingAttached()->updateChange(-0.00005);
    BuildingAttached()->updateActionPoint(3);
...
void AddBaffle::takeEffect() {
    BuildingAttached()->updateChange(-0.0001);
    BuildingAttached()->updateActionPoint(1);
    BuildingAttached()->addBaffle();
}
